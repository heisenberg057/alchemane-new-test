...
import { WIGS_COMPARE } from './content';
import styles from './AlchemaneWigsCompareSection.module.css';

export function AlchemaneWigsCompareSection() {
  const { titlePrefix, titleEmphasis, otherHead, usHead, captionOther, ctaLabel, rows } = WIGS_COMPARE;

  return (
    <section className="section section--white" aria-labelledby="compare-title">
      <div className="container">
        <AnimateOnScroll className={styles.head}>
          <h2 className="sectionTitle" id="compare-title">{titlePrefix} <em>{titleEmphasis}</em></h2>
        </AnimateOnScroll>

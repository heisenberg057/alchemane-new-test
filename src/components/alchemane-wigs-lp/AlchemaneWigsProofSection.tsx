...
import { WIGS_AWARD_IMAGE, WIGS_PROOF_STATS } from './content';
import styles from './AlchemaneWigsProofSection.module.css';

export function AlchemaneWigsProofSection({ mediaBase }: { mediaBase: string }) {
  return (
      ...
      <div className="container">
        ...
        <AnimateOnScroll>
          <dl className={styles.stats}>
            {WIGS_PROOF_STATS.map((stat, index) => (
              <div key={index}>
                <dt>{stat.label}</dt>
                <dd>{stat.value}</dd>
              </div>
        ...
        <AnimateOnScroll>
          <figure className={styles.award}>
            <div className={styles.photo}>
              <MediaImage
                src={`${mediaBase}/${WIGS_AWARD_IMAGE.src780}`}
                alt={WIGS_AWARD_IMAGE.alt}
                width={780}
                height={438}

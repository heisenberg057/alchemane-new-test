...
import { TOPPERS_WHO_ARIA_LABEL, TOPPERS_WHO_ITEMS } from './content';
import styles from './AlchemaneToppersWhoSection.module.css';
import { AlchemaneCtaButton } from '@/components/alchemane/AlchemaneCtaButton';

export function AlchemaneToppersWhoSection({ mediaBase }: { mediaBase: string }) {
  const count = TOPPERS_WHO_ITEMS.length;
  ...
  return (
    <section className="section section--white" id="who" aria-labelledby="alc-who-title">
      ...
      <div className="container">
        <div className={styles.carousel} role="group" aria-roledescription="carousel" aria-label={TOPPERS_WHO_ARIA_LABEL}>
          <div ref={scrollRef} className={styles.track}>
            {TOPPERS_WHO_ITEMS.map((item, index) => (

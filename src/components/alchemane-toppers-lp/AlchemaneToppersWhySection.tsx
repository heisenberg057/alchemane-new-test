...
import { TOPPERS_BENEFITS, TOPPERS_WHY } from './content';
import styles from './AlchemaneToppersWhySection.module.css';
import { AlchemaneCtaButton } from '@/components/alchemane/AlchemaneCtaButton';

export function AlchemaneToppersWhySection({ mediaBase }: { mediaBase: string }) {
  const { playing, handlePlay } = usePlayableVideo();
  const { kicker, title, titleEmphasis, video, ctaLabel } = TOPPERS_WHY;
          ...
          <div className={styles.body}>
            <AnimateOnScroll>
              <ol className={styles.benefits}>
                {TOPPERS_BENEFITS.map((benefit, index) => (
                  <li key={index}>

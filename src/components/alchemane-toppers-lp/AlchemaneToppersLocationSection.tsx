...
import { TOPPERS_LOCATION_PHOTOS } from './content';
import styles from './AlchemaneToppersLocationSection.module.css';

export function AlchemaneToppersLocationSection({ mediaBase }: { mediaBase: string }) {
  const count = TOPPERS_LOCATION_PHOTOS.length;
      ...
      <div className="container">
        <div className={styles.carousel} role="group" aria-roledescription="carousel" aria-label="Studio photos">
          <div ref={scrollRef} className={styles.track}>
            {TOPPERS_LOCATION_PHOTOS.map((photo, index) => (
              <figure className={styles.card} key={index} role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${count}`}>
                <MediaImage
          ...
          <div className="carouselNav">
            ...
            <div className="carouselProgress" aria-label="Select studio photo">
              {TOPPERS_LOCATION_PHOTOS.map((_, index) => (

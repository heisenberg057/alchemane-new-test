...
import { EXTENSION_CONSULT_CLIPS, EXTENSION_CONSULT_HINT } from './content';
import styles from './AlchemaneExtensionConsultSection.module.css';
import { AlchemaneCtaButton } from '@/components/alchemane/AlchemaneCtaButton';

function ConsultClip({ mediaBase, index, video }: { mediaBase: string; index: number; video: (typeof EXTENSION_CONSULT_CLIPS)[number] }) {
  const { playing, handlePlay } = usePlayableVideo();
  return (
    <article className={styles.card} role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${EXTENSION_CONSULT_CLIPS.length}`}>
      <MediaVideo
...
export function AlchemaneExtensionConsultSection({ mediaBase }: { mediaBase: string }) {
  const count = EXTENSION_CONSULT_CLIPS.length;
        ...
        <div className={styles.carousel}>
          ...
          <div className="carouselNav">
            ...
            <div className="carouselProgress" aria-label="Select consultation clip">
              {EXTENSION_CONSULT_CLIPS.map((_, index) => (

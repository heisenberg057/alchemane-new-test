...
import { EXTENSION_FAQ } from './content';
import styles from './AlchemaneExtensionFaqSection.module.css';
...
export function AlchemaneExtensionFaqSection() {
  const [active, setActive] = useState(0);
  const tabRefs = useRef<(HTMLButtonElement | null)[]>([]);

  const onKeyDown = (event: KeyboardEvent<HTMLButtonElement>, index: number) => {
    let target: number | null = null;
    if (event.key === 'ArrowRight' || event.key === 'ArrowDown') target = (index + 1) % EXTENSION_FAQ.tabs.length;
    else if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') target = (index - 1 + EXTENSION_FAQ.tabs.length) % EXTENSION_FAQ.tabs.length;
    else if (event.key === 'Home') target = 0;
    else if (event.key === 'End') target = EXTENSION_FAQ.tabs.length - 1;
    if (target !== null) {

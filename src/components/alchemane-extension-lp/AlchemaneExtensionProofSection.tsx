...
import { EXTENSION_AWARD_IMAGE, EXTENSION_PROOF_STATS } from './content';
import styles from './AlchemaneExtensionProofSection.module.css';

export function AlchemaneExtensionProofSection({ mediaBase }: { mediaBase: string }) {
  return (
      ...
      <div className="alcContainer">
        ...
        <AnimateOnScroll>
          <dl className={styles.stats}>
            {EXTENSION_PROOF_STATS.map((stat, index) => (
              <div key={index}>
                <dt>{stat.label}</dt>
                <dd>{stat.value}</dd>
              </div>
        ...
        <AnimateOnScroll>
          <figure className={styles.award}>
            <div className={styles.photo}>
              <MediaImage
                src={`${mediaBase}/${EXTENSION_AWARD_IMAGE.src780}`}
                alt={EXTENSION_AWARD_IMAGE.alt}
                width={780}
                height={438}

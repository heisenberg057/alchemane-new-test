...
import { EXTENSION_FOUNDER } from './content';
import styles from './AlchemaneExtensionFounderSection.module.css';
import { AlchemaneCtaButton } from '@/components/alchemane/AlchemaneCtaButton';

export function AlchemaneExtensionFounderSection({ mediaBase }: { mediaBase: string }) {
  const { playing, handlePlay } = usePlayableVideo();
  const { background, credentials, ctaLabel, video } = EXTENSION_FOUNDER;

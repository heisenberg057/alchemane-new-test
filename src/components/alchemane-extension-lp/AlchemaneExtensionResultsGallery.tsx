...
import { EXTENSION_GALLERY_CLIPS } from './content';
import styles from './AlchemaneExtensionResultsGallery.module.css';

function ClipCard({ mediaBase, index, video }: { mediaBase: string; index: number; video: (typeof EXTENSION_GALLERY_CLIPS)[number] }) {
  const { playing, handlePlay } = usePlayableVideo();
  return (
    <article className={styles.card} role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${EXTENSION_GALLERY_CLIPS.length}`}>
...
export function AlchemaneExtensionResultsGallery({ mediaBase }: { mediaBase: string }) {
  const count = EXTENSION_GALLERY_CLIPS.length;
      ...
      <div className="alcContainer">
        <div className={styles.carousel} role="group" aria-roledescription="carousel" aria-label="Real client results">
          <div ref={scrollRef} className={styles.track}>
            {EXTENSION_GALLERY_CLIPS.map((video, index) => (
          ...
          <div className="carouselNav">
            ...
            <div className="carouselProgress" aria-label="Select client story">
              {EXTENSION_GALLERY_CLIPS.map((_, index) => (

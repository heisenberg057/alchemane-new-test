...
import { EXTENSION_METHODS, EXTENSION_METHODS_HINT } from './content';
import styles from './AlchemaneExtensionMethodsSection.module.css';

function MethodCard({ mediaBase, index, item }: { mediaBase: string; index: number; item: (typeof EXTENSION_METHODS)[number] }) {
  const { playing, handlePlay } = usePlayableVideo();
  return (
    <article className={styles.card} role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${EXTENSION_METHODS.length}`}>
      <MediaVideo
...
export function AlchemaneExtensionMethodsSection({ mediaBase }: { mediaBase: string }) {
  const count = EXTENSION_METHODS.length;
      ...
      <div className="alcContainer">
        <div className={styles.carousel}>
          ...
          <div className="carouselNav">
            <button type="button" className="carouselControl" disabled={!canPrev} aria-label="Previous method" onClick={scrollPrev}>
              <svg aria-hidden="true"><use href="#alc-i-chev-left" /></svg>
            </button>
            <div className="carouselProgress" aria-label="Select application method">
              {EXTENSION_METHODS.map((_, index) => (
            ...
            <button type="button" className="carouselControl" disabled={!canNext} aria-label="Next method" onClick={scrollNext}>

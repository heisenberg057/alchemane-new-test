/**
 * Stage + upload Bangalore LP stills to Cloudflare R2.
 *
 ...
 *
 * Usage:
 *   node scripts/upload-bangalore.mjs
 *   DRY_RUN=1 node scripts/upload-bangalore.mjs
 */
import fs from 'fs'
...
const SOURCE_DIR = path.join(projectRoot, 'public', 'assets', 'bangalore-lp')
const LOCAL_DIR = path.join(projectRoot, 'public', 'media', 'bangalore-lp')
const R2_PREFIX = 'media/bangalore-lp'
...
fs.mkdirSync(path.dirname(LOCAL_DIR), { recursive: true })
fs.cpSync(SOURCE_DIR, LOCAL_DIR, { recursive: true })
...
console.log(`Staged ${fileCount} files → ${LOCAL_DIR}`)
